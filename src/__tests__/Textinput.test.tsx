...
const mockMaliciousAttemptLogs = jest.fn();
jest.mock('protecjs/src/db/influxdb.ts', () => ({
  maliciousAttemptLogs: mockMaliciousAttemptLogs,
}));
const Wrapper = ({ dompurifyConfig }: { dompurifyConfig?: { [key: string]: any } }) => {
  ...
  const onChange = (newValue: string) => {
    ...
    mockMaliciousAttemptLogs('XSS', { input_data: newValue });
  };
  return <TextInput value={value} onChange={onChange} dompurify={dompurifyConfig} />;
};


test("calls maliciousAttemptsLogs when detect malicious input",()=>{
  render(<Wrapper/>);
 ...
 expect(mockMaliciousAttemptLogs).toHaveBeenCalledWith("XSS",{
  input_data:maliciousInput,
 })
});
